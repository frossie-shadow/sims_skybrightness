...
# Make a function that fits everything simultaneously.  Prob need a
# pixel-by-pixel additive constant, and mag.  Maybe slope depends only on
# az?
...
for filterName in filters:
    ...
    s = ax.scatter(np.degrees(xdata['sunAlt']), resids,
                   c=np.degrees(xdata['alt']), alpha=.2, edgecolor='none')
    ...
    s = ax.scatter(np.degrees(xdata['sunAlt']), resids2,
                   c=np.degrees(xdata['alt']), alpha=.2, edgecolor='none')
    ax.set_xlabel('Sun Altitude (degrees)')
    ax.set_ylabel('Model - Data (mags)')
